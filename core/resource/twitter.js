  ...
  var $twt = {
    ...
    init: function (config) {
      ...
      if (!$twt.config.access_token_key) {
        throw new Error("no 'access_token_key' key found in resource '" + $twt.id + "' config");
      }
      if (!$twt.config.access_token_secret) {
        throw new Error("no 'access_token_secret' key found in resource '" + $twt.id + "' config");
      }
    ...
    open: function (callback) {
      ...
      $timer.start(timerId);
      var config = {
        access_token_key: $twt.config.access_token_key,
        access_token_secret: $twt.config.access_token_secret
      };
      if ($twt.config.consumer_key) {
        config.consumer_key = $twt.config.consumer_key;
      }
      if ($twt.config.consumer_secret) {
        config.consumer_secret = $twt.config.consumer_secret;
      }
      $twt.connection = new $twt.api.Twitter(config);
